...
interface DoorStatus {
  ...
  // ✅ Automático (viene desde door_status)
  auto_on?: boolean | null
  auto_last_updated?: string | null
...
export function DashboardMonitor() {
  ...
  const fetchStatus = async () => {
    try {
      ...
      const list = Array.isArray(data) ? (data as DoorStatus[]) : []
      setDoors(list)
    } catch (error) {
  ...
  useEffect(() => {
    fetchStatus()
    const interval = setInterval(fetchStatus, 2000) // refresco rápido
            ...
            >
              ...
              <CardContent>
                <div className="space-y-2">
                  ...
                  {/* ✅ AUTOMÁTICO (sale directo de door_status) */}
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">Automático</span>

                    {door.auto_on === null || door.auto_on === undefined ? (
                      <Badge variant="outline" className="bg-transparent">
                        Sin dato
                      </Badge>
                    ) : (
                      <Badge
                        variant={door.auto_on ? "default" : "secondary"}
                        className={door.auto_on ? "bg-green-500" : "bg-zinc-500"}
                      >
                        {door.auto_on ? "Arriba (ON)" : "Abajo (OFF)"}
                      </Badge>
                    )}
                  </div>

                  {door.auto_last_updated && (
                    <div className="text-xs text-muted-foreground">
                      Auto actualizado: {new Date(door.auto_last_updated).toLocaleString("es-CL")}
                    </div>
                  )}

                  {/* Duración si está abierta */}
